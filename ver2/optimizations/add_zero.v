...
Lemma optimize_add_0_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_add_0_sbinding.
Proof.
...
+ destruct label eqn: eq_label; try 
  ...
  destruct r2 as [|arg3 r3] eqn: eq_r2; try 
    (injection Hoptm_add_0_sbinding as eq_val' eq_flag;
    rewrite <- eq_val'; assumption).
  destruct (fcmp arg1 (Val WZero) n sb n sb instk_height ops)
    eqn: eq_fcmp_arg1.
  * injection Hoptm_add_0_sbinding as eq_val' eq_flag.
    rewrite <- eq_val'.
    simpl in Hvalid_smapv_val. unfold valid_stack_op_instr in Hvalid_smapv_val.
    destruct (ops ADD) as [nargs f H1 H2].
    destruct Hvalid_smapv_val as [_ Hvalid_args].
    simpl in Hvalid_args. destruct Hvalid_args as [_ [Hvalid_arg2 _]].
    simpl.
    assumption.
  * destruct (fcmp arg2 (Val WZero) n sb n sb instk_height 
        ops) eqn: eq_fcmp_arg2; try 
      (injection Hoptm_add_0_sbinding as eq_val' eq_flag;
      rewrite <- eq_val'; assumption).
    injection Hoptm_add_0_sbinding as eq_val' eq_flag.
    rewrite <- eq_val'.
    ...
    destruct (ops ADD) as [nargs f H1 H2].
    destruct Hvalid_smapv_val as [_ Hvalid_args].
    simpl in Hvalid_args. destruct Hvalid_args as [Hvalid_arg1 [_ _]].
    simpl.
    assumption.
  + injection Hoptm_add_0_sbinding as eq_val' eq_flag.
    rewrite <- eq_val'.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_add_0_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_add_0_sbinding_smapv_valid. 
...
Definition optimize_add_0 (fcmp: sstack_val_cmp_t): optim := 
optimize_first_sstate optimize_add_0_sbinding fcmp.
...
apply optimize_first_snd_opt_snd.
- assumption.
