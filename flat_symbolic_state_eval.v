...
Fixpoint eval_sexpr (fsv : sexpr) (stk: stack) (mem: memory) (strg: storage) (exts: externals) (ops : stack_op_instr_map) : option EVMWord :=
  match fsv with
  ...
  | SExpr_Op label args  =>
      ...
      | OpImp nargs f _ _ =>
          ...
          if (List.length args =? nargs) then
            let f_eval_list := fun (fsv': sexpr) => eval_sexpr fsv' stk mem strg exts ops in
  ...
  | SExpr_PUSHTAG v =>
      let tags := (get_tags_exts exts) in Some (tags v)
  | SExpr_MLOAD offset smem =>
      let f_eval_mem_update := instantiate_memory_update (fun sv => eval_sexpr sv stk mem strg exts ops) in
      match map_option f_eval_mem_update smem with (* Evaluate the arguments of the updates *)
      | None => None
      | Some mem_updates =>
          match eval_sexpr offset stk mem strg exts ops with (* Evaluate the offset *)
          | None => None
          | Some offset =>
  ...
  | SExpr_SLOAD key sstrg => 
      let f_eval_strg_update := instantiate_storage_update (fun sv => eval_sexpr sv stk mem strg exts ops) in
      match map_option f_eval_strg_update sstrg with (* Evaluate the arguments of the updates *)
      | None => None
      | Some strg_updates =>
          match eval_sexpr key stk mem strg exts ops with (* Evaluate the key *)
          | None => None
          | Some key =>
  ...
  | SExpr_SHA3 offset size smem => 
      ...
      | Some mem_updates =>
          ...
          | None => None
          | Some offset =>
              match eval_sexpr size stk mem strg exts ops with (* Evaluate the size *)
              | None => None
              | Some size =>
                  let mem := update_memory mem mem_updates in (* apply updates to the memory *)
...
Definition eval_flat_sstack (fsstk: flat_sstack) (stk: stack) (mem: memory) (strg: storage) (exts: externals) (ops: stack_op_instr_map) : option stack :=
  map_option (fun sv => eval_sexpr sv stk mem strg exts ops) fsstk.

Definition eval_flat_smemory (fsmem: flat_smemory) (stk: stack) (mem: memory) (strg: storage) (exts: externals) (ops: stack_op_instr_map) : option memory :=
  let f_eval_mem_update := instantiate_memory_update (fun sv => eval_sexpr sv stk mem strg exts ops) in
...
Definition eval_flat_sstorage (fsstrg: flat_sstorage) (stk: stack) (mem: memory) (strg: storage) (exts: externals) (ops: stack_op_instr_map) : option storage :=
  let f_eval_strg_update := instantiate_storage_update (fun sv => eval_sexpr sv stk mem strg exts ops) in
...
Definition eval_flat_sstate (st: state) (fsst: flat_sstate) (ops: stack_op_instr_map) : option state :=
  ...
  if instk_height =? length stk then
    match eval_flat_sstack fsstk stk mem strg exts ops with
    | None => None
    | Some stk' =>
        match eval_flat_smemory fsmem stk mem strg exts ops with
        | None => None
        | Some mem' =>
            match eval_flat_sstorage fsstrg stk mem strg exts ops with
            | None => None
            | Some strg' =>
