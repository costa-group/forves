...
(* OR(X,0) or OR(0,X) = X *)
Definition optimize_or_zero_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma optimize_or_zero_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_or_zero_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
unfold optimize_or_zero_sbinding in Hoptm_sbinding.
...
Lemma evm_or_zero_l: forall exts v,
evm_or exts [WZero; v] = v.
Proof.
intros exts v. simpl.
rewrite -> wor_wzero.
reflexivity.
...
Lemma evm_or_zero_r: forall exts v,
evm_or exts [v; WZero] = v.
Proof.
intros exts v. simpl.
rewrite -> wor_comm.
rewrite -> wor_wzero.
...
Lemma optimize_or_zero_sbinding_snd:
opt_sbinding_snd optimize_or_zero_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_or_zero_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_or_zero_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  + (* arg1 ~ WZero *)
    ...
    rewrite -> evm_or_zero_l in Heval_orig.
    rewrite <- Heval_orig.
  ...
  + (* arg2 ~ WZero *)
    ...
    * injection Hoptm_sbinding as eq_val' _.
      ...
      rewrite <- eq_vzero in Heval_orig.
      rewrite -> evm_or_zero_r in Heval_orig.
      rewrite <- Heval_orig.
      rewrite <- eval_sstack_val'_freshvar.
