...
Module Opt_lt_zero_bool.


(* LT(0, SLT(X,Y)) = SLT(X,Y) *)
Definition optimize_lt_zero_bool_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
| SymOp LT [arg1; arg2] => 
  ...
  | Some (FollowSmapVal (SymOp SLT args) idx' sb') => 
      match fcmp arg1 (Val WZero) maxid sb maxid sb instk_height ops with
      | true => (SymOp SLT args, true)
      | _ => (val, false)
      end
  | _ => (val, false)
  end
...
(*
Lemma evm_lt_zero_bool_snd: forall exts x y,
...
Lemma optimize_lt_zero_bool_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_lt_zero_bool_sbinding.
Proof.
Admitted.
(*
...
unfold optimize_lt_zero_bool_sbinding in Hoptm_sbinding.
...
Lemma optimize_lt_zero_bool_sbinding_snd:
opt_sbinding_snd optimize_lt_zero_bool_sbinding.
Proof.
Admitted.
(*
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_lt_zero_bool_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_lt_zero_bool_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  rewrite <- eq_arg1_v. 
  rewrite <- evm_lt_zero_bool_snd.
  reflexivity.
Qed.
*)

End Opt_lt_zero_bool.
