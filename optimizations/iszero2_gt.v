...
Definition optimize_iszero2_gt_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
| SymOp ISZERO [arg1] => 
  ...
  | Some (FollowSmapVal (SymOp ISZERO [arg2]) idx' sb') => 
      ...
      | Some (FollowSmapVal (SymOp GT [x;y]) idx'' sb'') => 
          (SymOp GT [x;y], true)
      | _ => (val, false)
      end
  | _ => (val, false)
...
Lemma optimize_iszero2_gt_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_iszero2_gt_sbinding.
Proof.
...
pose proof (valid_follow_in_smap sb' arg2 instk_height idx' evm_stack_opm
  (SymOp GT [x;y]) idx'' sb'' Hvalid_arg2 Hvalid_sb' eq_follow_arg2)
  as [Hvalid_arg3 [Hvalid_sb'' Himpl3]].
pose proof (not_basic_value_smv_symop GT [x;y]) as Hnot_basic3.
...
- (* evaluation is preserved *) 
  ...
  pose proof (follow_suffix sb' arg2 idx'(SymOp GT [x; y]) idx'' sb''
