...
Definition optimize_and_or_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
(* 
  ...
  AND(OR(Y, X), X) = X
 *)
Lemma and_or_bool: forall (b1 b2: bool),
b1 && (b1 || b2) = b1.
Proof.
intros b1 b2. destruct b1; (try destruct b2; try reflexivity).
Qed.

Lemma wand_wor_1: forall (size: nat) (x y: word size),
wand x (wor x y) = x.
Proof.
dependent induction x.
- intros y. reflexivity.
- intros y. dependent destruction y.
  unfold wand. unfold bitwp. simpl.
  rewrite and_or_bool.
  fold bitwp. fold wand. fold wor.
  rewrite -> IHx.
  reflexivity.
Qed.

Lemma wand_wor_2: forall (size: nat) (x y: word size),
wand x (wor y x) = x.
Proof.
intros size x y.
rewrite -> wor_comm.
...
Lemma wand_wor_3: forall (size: nat) (x y: word size),
wand (wor x y) x = x.
Proof.
intros size x y.
rewrite wand_comm.
...
Lemma wand_wor_4: forall (size: nat) (x y: word size),
wand (wor y x) x = x.
Proof.
intros size x y.
rewrite -> wor_comm.
...
Lemma optimize_and_or_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_and_or_sbinding.
Proof.
...
- (* valid_sbindings *)
  ...
  apply optimize_and_or_sbinding_smapv_valid. 
