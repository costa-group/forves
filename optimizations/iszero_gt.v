...
Definition is_gt_zero (sv: sstack_val) (fcmp: sstack_val_cmp_t) 
  (maxid instk_height: nat) (sb: sbindings) (ops: stack_op_instr_map) :=
match follow_in_smap sv maxid sb with 
| Some (FollowSmapVal (SymOp GT [arg1; arg2]) idx' sb') => 
    if fcmp arg2 (Val WZero) maxid sb maxid sb instk_height ops
    then Some arg1
    else None
| _ => None
end.
...
Definition optimize_iszero_gt_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma iszero_gt_zero_snd: forall exts x,
evm_iszero exts [evm_gt exts [x; WZero]] = evm_iszero exts [x].
Proof.
intros exts x.
simpl.
...
Lemma optimize_iszero_gt_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_iszero_gt_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid Hoptm_sbinding.
unfold optimize_iszero_gt_sbinding in Hoptm_sbinding.
...
destruct (is_gt_zero arg1 fcmp n instk_height sb evm_stack_opm) as [x|] 
  eqn: is_gt_zero_arg1.
- injection Hoptm_sbinding as eq_val' _.
  rewrite <- eq_val'.
  unfold is_gt_zero in is_gt_zero_arg1.
  ...
  destruct r2 as [|zerov r2']; try discriminate.
  ...
  injection is_gt_zero_arg1 as eq_xx. rewrite -> eq_xx in Hfollow_arg1.
  pose proof (valid_follow_in_smap sb arg1 instk_height n evm_stack_opm
    (SymOp GT [x; zerov]) idx' sb' Hvalid_arg1 Hvalid Hfollow_arg1) as Himpl.
  destruct Himpl as [Hvalid_gt [Hvalid_sb' Himpl]].
  pose proof (not_basic_value_smv_symop GT [x;zerov]) as Hnot_basic.
  apply Himpl in Hnot_basic.
  unfold valid_smap_value in Hvalid_gt. 
  unfold valid_stack_op_instr in Hvalid_gt.
  simpl in Hvalid_gt.
  destruct Hvalid_gt as [_ [Hvalid_x_idx' [Hvalid_zerov_idx' _]]].
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_iszero_gt_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_iszero_gt_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  destruct args as [|arg1 r1] eqn: eq_args; 
    try inject_rw Hoptm_sbinding eq_val'.
  destruct r1; try inject_rw Hoptm_sbinding eq_val'.
  destruct (is_gt_zero arg1 fcmp idx instk_height sb evm_stack_opm) 
    as [x|] eqn: is_gt_zero_arg1.
  + injection Hoptm_sbinding as eq_val' _.
    rewrite <- eq_val'.
    unfold is_gt_zero in is_gt_zero_arg1.
    ...
    destruct (eval_sstack_val' n xx stk mem strg exts idx' sb'
      evm_stack_opm) as [xxv|] eqn: eval_xx; try discriminate.
    destruct (eval_sstack_val' n zerov stk mem strg exts idx' sb'
      evm_stack_opm) as [zerovv|] eqn: eval_zerov; try discriminate.
    injection is_gt_zero_arg1 as eq_x.
    rewrite -> eq_x in eval_xx.
    
    pose proof (follow_suffix sb arg1 idx (SymOp GT [xx; zerov]) idx' sb'
    ...
    pose proof (valid_follow_in_smap sb arg1 instk_height idx evm_stack_opm
      (SymOp GT [xx; zerov]) idx' sb' Hvalid_arg1 Hvalid_sb Hfollow_arg1)
      as [Hvalid_smv [Hvalid_sb' Himpl]].
    pose proof (not_basic_value_smv_symop GT [xx;zerov]) as Hnot_basic.
    ...
    pose proof (Hsafe_sstack_val_cmp zerov (Val WZero) idx sb idx sb instk_height
      ...
      stk mem strg exts Hlen) as [vv [eval_zerov' eval_WZero]].
    ...
    pose proof (eval_sstack_val'_extend_sb instk_height maxidx stk mem strg
      exts idx sb sb' evm_stack_opm prefix Hvalid_sb sb_prefix zerov zerovv
    ...
    pose proof (iszero_gt_zero_snd exts xxv) as H. simpl in H.
