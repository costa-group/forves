...
Lemma eq_zero_iszero1: forall (x: EVMWord) ctx,
evm_eq ctx [WZero; x] = evm_iszero ctx [x].
Proof.
intros. unfold evm_iszero. 
rewrite -> eq_comm. reflexivity.
Qed.

Lemma eq_zero_iszero2: forall (x: EVMWord) ctx,
evm_eq ctx [x; WZero] = evm_iszero ctx [x].
Proof.
intros. unfold evm_iszero. 
rewrite -> eq_comm. reflexivity.
Qed.
