...
(* EXP(X,1) = X *)
Definition optimize_exp_x_one_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma optimize_exp_x_one_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_exp_x_one_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
unfold optimize_exp_x_one_sbinding in Hoptm_sbinding.
...
rewrite <- eq_val'.
simpl. simpl in Hvalid_smapv_val. 
unfold valid_stack_op_instr in Hvalid_smapv_val. simpl in Hvalid_smapv_val.
destruct Hvalid_smapv_val as [_ [Hvalid_arg1 Hvalid_arg2]].
assumption.
Qed.
...
Lemma optimize_exp_x_one_sbinding_snd:
opt_sbinding_snd optimize_exp_x_one_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_exp_x_one_sbinding)
...
- (* evaluation is preserved *) 
  ...
  rewrite -> evm_exp_x_one in Heval_orig.
  rewrite <- Heval_orig.
