...
(* XOR(X,0) or XOR(0,X) = X *)
Definition optimize_xor_zero_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma optimize_xor_zero_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_xor_zero_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
unfold optimize_xor_zero_sbinding in Hoptm_sbinding.
...
Lemma evm_xor_zero_l: forall exts v,
evm_xor exts [WZero; v] = v.
Proof.
intros exts v. simpl.
rewrite -> wxor_wzero.
reflexivity.
...
Lemma evm_xor_zero_r: forall exts v,
evm_xor exts [v; WZero] = v.
Proof.
intros exts v. simpl.
rewrite -> wxor_comm.
rewrite -> wxor_wzero.
...
Lemma optimize_xor_zero_sbinding_snd:
opt_sbinding_snd optimize_xor_zero_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_xor_zero_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_xor_zero_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  + (* arg1 ~ WZero *)
    ...
    rewrite -> evm_xor_zero_l in Heval_orig.
    rewrite <- Heval_orig.
  ...
  + (* arg2 ~ WZero *)
    destruct (fcmp arg2 (Val WZero) idx sb idx sb instk_height evm_stack_opm)
      eqn: fcmp_arg2_zero; try inject_rw Hoptm_sbinding eq_val'.
    injection Hoptm_sbinding as eq_val' _.
    rewrite <- eq_val'.
    ...
    rewrite <- eq_vzero in Heval_orig.
    rewrite -> evm_xor_zero_r in Heval_orig.
    rewrite <- Heval_orig.
    rewrite <- eval_sstack_val'_freshvar.
