...
Definition optimize_mul_zero_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma optimize_mul_zero_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_mul_zero_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
unfold optimize_mul_zero_sbinding in Hoptm_sbinding.
...
Lemma word_mul_0_x_is_0: forall (x: EVMWord) exts,
evm_mul exts [WZero; x] = WZero.
Proof.
intros x exts. simpl.
rewrite -> wmult_neut_l. reflexivity.
Qed.

Lemma word_mul_x_0_is_0: forall (x: EVMWord) exts,
evm_mul exts [x; WZero] = WZero.
Proof.
intros x exts. simpl.
...
Lemma optimize_mul_zero_sbinding_snd:
opt_sbinding_snd optimize_mul_zero_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_mul_zero_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_mul_zero_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  + (* arg1 ~ WZero *)
    ...
    rewrite -> word_mul_0_x_is_0 in Heval_orig.
    rewrite <- Heval_orig.
    rewrite <- eval_sstack_val'_freshvar.
    apply eval_sstack_val'_const.
  + (* arg2 ~ WZero *)
    ...
    * injection Hoptm_sbinding as eq_val' _.
      ...
      rewrite <- eq_vzero in Heval_orig.
      rewrite -> word_mul_x_0_is_0 in Heval_orig.
      rewrite <- Heval_orig.
      rewrite <- eval_sstack_val'_freshvar.
