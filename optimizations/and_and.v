...
Definition optimize_and_and_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma wand_wand_2: forall (x y: EVMWord),
wand x (wand y x) = wand x y.
Proof.
intros x y.
rewrite -> (wand_comm y x).
apply wand_wand_1.
Qed.

Lemma wand_wand_3: forall (x y: EVMWord),
wand (wand x y) x = wand x y.
Proof.
intros x y.
rewrite -> (wand_comm x y).
rewrite <- wand_assoc.
rewrite -> wand_x_x.
rewrite -> wand_comm.
reflexivity.
...
Lemma wand_wand_4: forall (x y: EVMWord),
wand (wand y x) x = wand x y.
Proof.
intros x y.
rewrite -> (wand_comm y x).
apply wand_wand_3.
...
Lemma optimize_and_and_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_and_and_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid Hoptm_sbinding.
unfold optimize_and_and_sbinding in Hoptm_sbinding.
...
Lemma optimize_and_and_sbinding_snd:
opt_sbinding_snd optimize_and_and_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_and_and_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_and_and_sbinding_smapv_valid. 
