...
(* SUB(X,0)  = X *)
Definition optimize_sub_zero_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma optimize_sub_zero_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_sub_zero_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
unfold optimize_sub_zero_sbinding in Hoptm_sbinding.
...
Lemma evm_sub_zero_r: forall exts v,
evm_sub exts [v; WZero] = v.
Proof.
intros exts v. simpl.
rewrite -> wminus_def. rewrite -> wzero_wneg. 
rewrite wplus_comm. rewrite -> wzero_wplus. 
reflexivity.
Qed.


Lemma optimize_sub_zero_sbinding_snd:
opt_sbinding_snd optimize_sub_zero_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_sub_zero_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_sub_zero_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  * injection Hoptm_sbinding as eq_val' _.
    ...
    rewrite -> evm_sub_zero_r in Heval_orig.
    rewrite <- Heval_orig.
