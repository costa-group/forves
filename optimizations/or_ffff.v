...
(* OR(X,0) or OR(0,X) = 0 *)
Definition optimize_or_ffff_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma optimize_or_ffff_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_or_ffff_sbinding.
Proof.
...
Lemma optimize_or_ffff_sbinding_snd:
opt_sbinding_snd optimize_or_ffff_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_or_ffff_sbinding)
...
- (* evaluation is preserved *) 
  ...
  + (* arg1 ~ FFFF *)
    ...
    rewrite -> wor_ffff_x_ffff in Heval_orig.
    rewrite <- Heval_orig.
  ...
  + destruct (fcmp arg2 (Val (wones EVMWordSize)) idx sb idx sb instk_height evm_stack_opm)
      eqn: fcmp_arg2_ffff; try inject_rw Hoptm_sbinding eq_val'.
    (* arg2 ~ FFFF *)
    injection Hoptm_sbinding as eq_val'.
    rewrite <- wones_evm in eq_val'.
    rewrite <- eq_val'.
