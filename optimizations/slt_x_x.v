...
(* SLT(X,X) = 0 *)
Definition optimize_slt_x_x_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma optimize_slt_x_x_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_slt_x_x_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
unfold optimize_slt_x_x_sbinding in Hoptm_sbinding.
...
Lemma slt_x_x_zero: forall (x: EVMWord) exts,
evm_slt exts [x; x] = WZero.
Proof.
...
Lemma optimize_slt_x_x_sbinding_snd:
opt_sbinding_snd optimize_slt_x_x_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_slt_x_x_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_slt_x_x_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  rewrite -> slt_x_x_zero in Heval_orig.
  rewrite <- Heval_orig.
