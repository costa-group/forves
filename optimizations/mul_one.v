...
Definition optimize_mul_one_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
| SymOp MUL [arg1; arg2] => 
  ...
  else if fcmp arg2 (Val WOne) maxid sb maxid sb instk_height ops then
...
Lemma optimize_mul_one_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_mul_one_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
unfold optimize_mul_one_sbinding in Hoptm_sbinding.
...
Lemma word_mul_1_x_is_x: forall (x: EVMWord) exts,
evm_mul exts [WOne; x] = x.
Proof.
intros x. simpl.
rewrite -> wmult_unit. reflexivity.
Qed.

Lemma word_mul_x_1_is_x: forall (x: EVMWord) exts,
evm_mul exts [x; WOne] = x.
Proof.
intros x. simpl.
...
Lemma optimize_mul_one_sbinding_snd:
opt_sbinding_snd optimize_mul_one_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_mul_one_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_mul_one_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  + (* arg1 ~ WOne *)
    ...
    rewrite -> word_mul_1_x_is_x in Heval_orig.
    rewrite <- Heval_orig.
  ...
  + (* arg2 ~ WOne *)
    ...
    * injection Hoptm_sbinding as eq_val' _.
      ...
      rewrite <- eq_vzero in Heval_orig.
      rewrite -> word_mul_x_1_is_x in Heval_orig.
      rewrite <- Heval_orig.
      rewrite <- eval_sstack_val'_freshvar.
