...
(* EXP(1,X) = 1 *)
Definition optimize_exp_one_x_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
Lemma optimize_exp_one_x_sbinding_smapv_valid:
opt_smapv_valid_snd optimize_exp_one_x_sbinding.
Proof.
unfold opt_smapv_valid_snd.
intros instk_height n fcmp sb val val' flag.
intros _ Hvalid_smapv_val Hvalid_sb Hoptm_sbinding.
unfold optimize_exp_one_x_sbinding in Hoptm_sbinding.
...
Lemma evm_exp_one_x: forall exts (x: EVMWord), evm_exp exts [WOne; x] = WOne.
Proof.
intros. simpl. unfold wordBin.
rewrite -> N.pow_1_l. reflexivity.
Qed.


Lemma optimize_exp_one_x_sbinding_snd:
opt_sbinding_snd optimize_exp_one_x_sbinding.
Proof.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_exp_one_x_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_exp_one_x_sbinding_smapv_valid. 
...
- (* evaluation is preserved *) 
  ...
  destruct (fcmp arg1 (Val WOne) idx sb idx sb instk_height) 
    eqn: fcmp_arg1; try inject_rw Hoptm_sbinding eq_val'.
  ...
  pose proof (Hsafe_sstack_val_cmp arg1 (Val WOne) idx sb idx sb 
    instk_height evm_stack_opm Hvalid_arg1 Hvalid_v Hvalid_bindings_sb
    Hvalid_bindings_sb fcmp_arg1 stk mem strg exts Hlen2)
    as [vone [Heval_arg1 Heval_vone]].
      
  unfold eval_sstack_val in Heval_vone.
  rewrite -> eval_sstack_val'_const in Heval_vone.
  ...
  rewrite -> evm_exp_one_x in Heval_orig.
  rewrite <- Heval_orig.
