...
Module Opt_iszero2_slt.


(* ISZERO(ISZERO(SLT(X,Y)) = SLT(X,Y) *)
Definition optimize_iszero2_slt_sbinding : opt_smap_value_type := 
fun (val: smap_value) =>
...
| SymOp ISZERO [arg1] => 
  ...
  | Some (FollowSmapVal (SymOp ISZERO [arg2]) idx' sb') => 
      ...
      | Some (FollowSmapVal (SymOp SLT [x;y]) idx'' sb'') => 
          (SymOp SLT [x;y], true)
      | _ => (val, false)
      end
  | _ => (val, false)
...
unfold optimize_iszero2_slt_sbinding in Hoptm_sbinding.
...
split.
- (* valid_sbindings *)
  apply valid_bindings_snd_opt with (val:=val)(opt:=optimize_iszero2_slt_sbinding)
    (fcmp:=fcmp)(flag:=flag); try assumption.
  apply optimize_iszero2_slt_sbinding_smapv_valid. 
